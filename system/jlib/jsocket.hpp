...
public:
    ...
    virtual size32_t write2(void const* buf, size32_t size) = 0; // returns amount written normally same as in size (see set_nonblock)
    virtual size32_t writetms(void const* buf, size32_t size, unsigned timeoutms=WAIT_FOREVER) = 0;
    virtual size32_t writetms2(void const* buf, size32_t size, unsigned timeoutms=WAIT_FOREVER) = 0;
