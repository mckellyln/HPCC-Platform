...
#  endif
#  define _USE_LLIST // new linked-list method
# endif
#endif
...
#define CHECKSOCKIDRANGE(s, max) { if ((s<0)||(s>=(int)max)) THROWJSOCKEXCEPTION2(JSOCKERR_handle_too_large); }
...
bool CSocket::connect_timeout( unsigned timeout, bool noexception)
{
    ...
    while (!tm.timedout(&remaining))
    {
        ...
        if ((err == JSE_INPROGRESS)||(err == JSE_WOULDBLOCK))
        {
            ...
            if (rc>0)
            {
                // poll succeeded - return error from socket (0 if connected)
                socklen_t errlen = sizeof(err);
...
# else
            CHECKSOCKIDRANGE(dummysock[0], max_sock_id);
...
# else
            CHECKSOCKIDRANGE(dummysock, max_sock_id);
...
# else
        ...
        if (!getrlimit(RLIMIT_NOFILE, &rlim))
        {
            if (rlim.rlim_cur == RLIM_INFINITY)
                ...
                max_sock_id = rlim.rlim_cur;
        }
        else
            max_sock_id = 50000;
#  ifdef EPOLLTRACE
        DBGLOG("CSocketEpollThread: max socket id: %u", max_sock_id );
#  endif
# endif
...
# ifndef _USE_LLIST
        try {
            epfdtbl = new int[max_sock_id];
        } catch (const std::bad_alloc &e) {
            int err = ERRNO();
            LOGERR(err,1,"epfdtbl alloc()");
            THROWJSOCKEXCEPTION2(err);
        }
        for (unsigned i=0; i<max_sock_id; i++)
            epfdtbl[i] = -1;
# endif
        try {
            epevents = new struct epoll_event[max_sock_id];
        } catch (const std::bad_alloc &e) {
            int err = ERRNO();
...
#ifdef _USE_LLIST
        for (std::list<SelectItem*>::iterator iter = selList.begin(); iter != selList.end();)
        {
            try
            {
                SelectItem *si = *iter;
...
# endif
    ...
    bool add(ISocket *sock,unsigned mode,ISocketSelectNotify *nfy)
    {
        ...
        if (n>=max_sock_id-1)   // leave 1 spare
            return false;
        SelectItem sn;
        sn.nfy = LINK(nfy);
        sn.sock = LINK(sock);
        sn.mode = (byte)mode;
        sn.handle = (T_SOCKET)sock->OShandle();
        CHECKSOCKIDRANGE(sn.handle, max_sock_id);
        sn.del = false;
        sn.add_epoll = true;
        items.append(sn);
#endif
    ...
    int run()
    {
        ...
        try {
            ...
            while (!terminating) {
                ...
                int n = ::epoll_wait(epfd, epevents, max_sock_id, 1000);
...
# ifndef _USE_LLIST
                                if (epfdtbl[tfd] < 0)
                                {
                                    WARNLOG("epoll event for invalid fd: index = %d, fd = %d, eventmask = %u", j, tfd, epevents[j].events);
                                    continue;
                                }
                                SelectItem *epsi = items.getArray(epfdtbl[tfd]);
# endif
                                if (!epsi->del)
                                {
    ...
    ISocket *wait(unsigned timems)
    {
        ...
        do {
            ...
            if (!connectdone&&(err==0))
            {
                ...
                {   // poll failed
                    err = ERRNO();
                    LOGERR(err,2,"CSocketConnectWait ::poll");
                }
            }
            if (err==0)
            {
                  ...
                  UnsignedArray &readySocks) //OUT  sockets ready
{
    ...
    else if (res == SOCKET_ERROR)
    {
        ...
        if (err != JSE_INTR)
        {
            throw MakeStringException(-1,"wait_multiple::poll error %d", err);
        }
    }
    return res;
}
